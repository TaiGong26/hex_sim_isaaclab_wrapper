...
def main() -> None:
    """Run the Maver X4 velocity-level ground-speed verification."""
    parser = argparse.ArgumentParser(
        description="Maver X4 velocity-level swerve ground-speed test (Ctrl+C to stop)")
    ...
    robot.start()
    print("[INFO]: Setup complete, starting X4 VEL ground-speed test (Ctrl+C to stop).", flush=True)
    ...
    try:
        while True:
            ...
            for name, (vx, vy, omega), gate in phases:
                ...
                print(f"[x4 VEL][loop {loop}] {name:8s} cmd=({vx},{vy},{omega}) "
